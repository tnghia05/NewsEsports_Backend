...
export class KalstropWsService implements OnModuleDestroy {
      ...
      this.ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          if (msg.type === 'next' && msg.payload?.data?.sportsMatchDefaultMarketsOddsUpdated) {
            const update: OddsUpdate = msg.payload.data.sportsMatchDefaultMarketsOddsUpdated;
  ...
  async subscribe(marketId: string, subId: string, cb: OddsCallback): Promise<void> {
    await this.connect();
