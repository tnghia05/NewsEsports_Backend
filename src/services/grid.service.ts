...
const GRID_KEY =
  process.env.GRID_API_KEY ?? 'p4EAhc88U9nO2nXz9sGhs30XCJOaIDpYS5gbo8SO';
...
export class GridService {
  ...
  async getSchedule(
    titleIds: string[] = ['28', '2'],
    dateFrom?: string,
    dateTo?: string,
  ) {
    const from =
      dateFrom ?? new Date(Date.now() - 3 * 24 * 3600_000).toISOString();
  ...
  async getScheduleWithScores(
    ...
    const now = Date.now();
    const past = series.filter(
      (s: any) => new Date(s.startTimeScheduled).getTime() < now - 3600_000,
    );
    ...
    return series.map((s: any) => {
      ...
      return {
        ...s,
        seriesScore: {
          a: st.teams?.[0]?.score ?? 0,
          b: st.teams?.[1]?.score ?? 0,
        },
